...
public class RotaCoordinatorTests : TestBase
{
	...
	{
		const string desc = "Who's planet?";
		var user = GetTestUser(EUserAccessLevel.RotaCoordinator);
		var session = DodoServer.SessionManager.GetOrCreateSession(user);
	...
	public async Task RemoveNeedFromMany()
	{
		...
		DodoServer.CoordinatorNeedsManager.AddNeedRequest(user, role.WorkingGroup, role.SiteCode, 4, DateTime.Now, "test1");
		DodoServer.CoordinatorNeedsManager.AddNeedRequest(user, role.WorkingGroup, role.SiteCode, 2, DateTime.Now, "test1");

		var msg = DodoServer.TelegramGateway.FakeMessage("DELETENEED", user.TelegramUser);
		msg = DodoServer.TelegramGateway.FakeMessage(DodoServer.CoordinatorNeedsManager.Data.CurrentNeeds.Keys.Random(), user.TelegramUser);
