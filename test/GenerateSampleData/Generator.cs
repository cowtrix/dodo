	...
	public static class Generator
	{
		...
		public static async Task GenerateRebellion(string city, AccessContext context)
		{
			...
			for(var i = 0; i < 8; ++ i)
			{
				...
				var actionOccupation = TestBase.CreateNewObject<Event>(context, new EventSchema("Protest For Nature", currentRebellion.Guid.ToString().ToString(), new GeoLocation(location.ToCoordinate().Latitude + jitter(), location.ToCoordinate().Longitude + jitter()), SchemaGenerator.SampleDescription, SchemaGenerator.RandomFacilities, SchemaGenerator.RandomVideoURL, rootDate() + TimeSpan.FromDays(1), rootDate() + TimeSpan.FromDays(2) + TimeSpan.FromHours(4)));
				var march = TestBase.CreateNewObject<Event>(context, new EventSchema("Youth March", currentRebellion.Guid.ToString().ToString(), new GeoLocation(location.ToCoordinate().Latitude + jitter(), location.ToCoordinate().Longitude + jitter()), SchemaGenerator.SampleDescription, SchemaGenerator.RandomFacilities, SchemaGenerator.RandomVideoURL, rootDate() + TimeSpan.FromDays(1), rootDate() + TimeSpan.FromDays(2) + TimeSpan.FromHours(4)));
				var sanctuary = TestBase.CreateNewObject<Site>(context, new SiteSchema("Second Occupation", currentRebellion.Guid.ToString().ToString(), new GeoLocation(location.ToCoordinate().Latitude + jitter(), location.ToCoordinate().Longitude + jitter()), SchemaGenerator.SampleDescription, SchemaGenerator.RandomFacilities, SchemaGenerator.RandomVideoURL));
				var evt = TestBase.CreateNewObject<Event>(context, new EventSchema("Activism Workshop", currentRebellion.Guid.ToString().ToString(), new GeoLocation(location.ToCoordinate().Latitude + jitter(), location.ToCoordinate().Longitude + jitter()), SchemaGenerator.SampleDescription, SchemaGenerator.RandomFacilities, SchemaGenerator.RandomVideoURL, rootDate() + TimeSpan.FromDays(1), rootDate() + TimeSpan.FromDays(2) + TimeSpan.FromHours(4)));
			}
