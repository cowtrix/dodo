...
namespace GenerateSampleData
{
	...
	public static class Generator
	{
		...
		public static async Task GenerateRebellion(string city, AccessContext context)
		{
			...
			TestBase.CreateNewObject<LocalGroup>(context, new LocalGroupSchema($"East {city}", SchemaGenerator.SampleDescription, new GeoLocation(location.Latitude, location.Longitude - .1 * rnd.NextDouble())), seed: false);
			TestBase.CreateNewObject<LocalGroup>(context, new LocalGroupSchema($"West {city}", SchemaGenerator.SampleDescription, new GeoLocation(location.Latitude, location.Longitude + .1 * rnd.NextDouble())), seed: false);
			TestBase.CreateNewObject<LocalGroup>(context, new LocalGroupSchema($"North {city}", SchemaGenerator.SampleDescription, new GeoLocation(location.Latitude - .1 * rnd.NextDouble(), location.Longitude)), seed: false);
			TestBase.CreateNewObject<LocalGroup>(context, new LocalGroupSchema($"South {city}", SchemaGenerator.SampleDescription, new GeoLocation(location.Latitude - .1 * rnd.NextDouble(), location.Longitude)), seed: false);

			DateTime rootDate() => DateTime.Today + TimeSpan.FromDays(rnd.Next(365)) + TimeSpan.FromHours(rnd.Next(20));
			...
			var actionOccupation = TestBase.CreateNewObject<Event>(context, new EventSchema("Protest For Nature", currentRebellion.Guid.ToString().ToString(), new GeoLocation(location.ToCoordinate().Latitude + 0.05 * rnd.NextDouble(), location.ToCoordinate().Longitude + 0.05), SchemaGenerator.SampleDescription, rootDate() + TimeSpan.FromDays(1), rootDate() + TimeSpan.FromDays(1) + TimeSpan.FromHours(4)));
			var march = TestBase.CreateNewObject<Event>(context, new EventSchema("Youth March", currentRebellion.Guid.ToString().ToString(), new GeoLocation(location.ToCoordinate().Latitude - 0.05 * rnd.NextDouble(), location.ToCoordinate().Longitude + 0.05), SchemaGenerator.SampleDescription, rootDate() + TimeSpan.FromDays(1), rootDate() + TimeSpan.FromDays(1) + TimeSpan.FromHours(4)));
			...
			var evt = TestBase.CreateNewObject<Event>(context, new EventSchema("Activism Workshop", currentRebellion.Guid.ToString().ToString(), new GeoLocation(location.ToCoordinate().Latitude - 0.05 * rnd.NextDouble(), location.ToCoordinate().Longitude - 0.05), SchemaGenerator.SampleDescription, rootDate() + TimeSpan.FromDays(1), rootDate() + TimeSpan.FromDays(1) + TimeSpan.FromHours(4)));
