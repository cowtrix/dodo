...
namespace Groups
{
	...
	public class RebellionTests : AdministratedGroupResourceTestBase<Rebellion> 
	{
		...
		public void CanAddAndRemoveSubWorkingGroup()
		{
			...
			Assert.IsTrue((newWg.Parent.GetValue(true) as Rebellion).WorkingGroups.Any(s => s.Guid == newWg.Guid),
				"Working Group was not included in Rebellion list after creation");
			ResourceUtility.GetManager<WorkingGroup>().Delete(newWg);
			Assert.IsFalse((newWg.Parent.GetValue(true) as Rebellion).WorkingGroups.Any(s => s.Guid == newWg.Guid),
		...
		public void CanAddAndRemoveSite()
		{
			...
			Assert.IsTrue((eventSite.Parent.GetValue(true) as Rebellion).Sites.Any(s => s.Guid == eventSite.Guid),
				"Site was not included in Rebellion list after creation");
			ResourceUtility.GetManager<Site>().Delete(eventSite);
			Assert.IsFalse((eventSite.Parent.GetValue(true) as Rebellion).WorkingGroups.Any(s => s.Guid == eventSite.Guid),
		...
		public void CanAddAndRemoveEvent()
		{
			...
			Assert.IsTrue((eventSite.Parent.GetValue(true) as Rebellion).Events.Any(s => s.Guid == eventSite.Guid),
				"Site was not included in Rebellion list after creation");
