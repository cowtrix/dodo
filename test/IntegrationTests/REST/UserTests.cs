...
namespace RESTTests
{
	...
	public class UserTests : IntegrationTestBase
	{
		...
		public async Task CanResetPassword()
		{
			...
			var token = user.TokenCollection.GetSingleToken<ResetPasswordToken>(context, EPermissionLevel.ADMIN, user);
		...
		public async Task CanVerifyEmail()
		{
			...
			var token = user.TokenCollection.GetSingleToken<VerifyEmailToken>(context, EPermissionLevel.ADMIN, user);
