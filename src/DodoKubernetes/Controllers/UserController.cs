...
namespace Dodo.Users
{
    ...
    public class UserController : ObjectRESTController<User, UserSchema>
    {
        ...
        [HttpPost("~/signin")]
        public Task<IActionResult> SignIn(string provider, string returnUrl)
        {
            // Instruct the middleware corresponding to the requested external identity
            // provider to redirect the user agent to its own authorization endpoint.
            // Note: the authenticationScheme parameter must match the value configured in Startup.cs
            return Task.FromResult<IActionResult>(Challenge(new AuthenticationProperties { RedirectUri = returnUrl }, provider));
        }
