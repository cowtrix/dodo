...
{
	public abstract class UserControllerBase : ResourceController<User, UserSchema>
	{
		...
		protected async Task<IActionResult> Login_Internal([FromBody] LoginModel login)
		{
		...
		protected async Task<IActionResult> Logout_Internal()
		{
			if (Context.User == null)
			{
				return Forbid();
		...
		protected async Task<IActionResult> RequestPasswordReset_Internal(string email)
		{
			...
			if (targetUser != null)
			{
				using (var rscLock = new ResourceLock(targetUser))
				{
					...
					EmailHelper.SendPasswordResetEmail(targetUser.PersonalData.Email, targetUser.Name,
						$"{Dodo.NetConfig.FullURI}/{RootURL}/{RESET_PASSWORD}?token={resetToken.Key}");
				}
			}
			return Ok();
		}

		protected async Task<IActionResult> ResetPassword_Internal(string token, [FromBody]string password)
		{
			...
			return Redirect(Dodo.NetConfig.FullURI);
		}

		protected async Task<IActionResult> ChangePassword_Internal([FromBody]ChangePasswordModel model)
		{
			if (Context.User == null)
			{
				return Forbid();
		...
		protected async Task<IActionResult> VerifyEmail_Internal(string token)
		{
			if (Context.User == null)
			{
				return Forbid();
		...
		protected async Task<IActionResult> Register_Internal([FromBody] UserSchema schema, [FromQuery]string token = null)
		{
			if(!string.IsNullOrEmpty(token))
			{
		...
		private static void SendEmailVerification(AccessContext context)
		{
			...
			EmailHelper.SendEmailVerificationEmail(context.User.PersonalData.Email, context.User.Name,
				$"{Dodo.NetConfig.FullURI}/{RootURL}/{VERIFY_EMAIL}?token={token.Token}");
		}
